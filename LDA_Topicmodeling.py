...
def read_uploaded_files(data_file):
...
def remove_chars(text):
    ...
    return re.sub(
        r'[$+&+;+]|[><!+،:,\(\).+،+٬+,+]|[-+]|[…]|[\[\]»«//]|[\\]|[#+]|[_+]|[٪+]|[%]|[*+]|[؟+]|[?+]|[""]|@|'
        "|[٠|١|۴|۵|۶|۷|۸|۲|۳|۰|٤|٣|٢|۹|٨|٧|٦|۱|٥]",
        "",
        str(text),
    )
...
def remove_unusful_topics(topics):
    return {
        ...
        if not x.endswith("های") and not x.endswith("هایی")
    }
...
def remove_nested_keywords(topics):
    ...
    for i in range(len(keys)):
        try:
            ...
            if (
                len(new_key.split()) > 1
                and new_key.split().count("_") > 1
                and not new_key.split()[-1] != "_"
                and not new_key.split()[0] != "_"
            ):
...
def run(df=None, stopwords=None, filename: str = None, num_topic: int = 0):
    if filename:
...
if __name__ == "__main__":
    ...
    case_list = run(filename="./2_Instagram_comment.csv", num_topic=5)
